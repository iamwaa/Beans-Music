...
> 第三方音源需自行在「设置 - 第三方音源」中添加或导入；开启蓝牙歌词后，当前歌词会同步到锁屏、控制中心和支持歌词显示的蓝牙设备。

### 新增与优化

- 新增蓝牙歌词模式：按播放进度获取当前歌词，并同步更新系统正在播放信息，便于车机、耳机和其他蓝牙设备显示歌词。
- 第三方音源支持 JSON 配置和 LX Music `.js` 音源脚本，作为官方播放地址不可用时的备用来源。
